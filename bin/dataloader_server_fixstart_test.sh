...
ssh ${node} kill $pid
    ...
    echo "afterkill"
    ssh ${node} ps aux|grep $main|awk '{print$2}'
    ssh ${node} nohup /usr/java/jdk/bin/java $fileencoding $memarg $gcarg $verboses -Dxa.nodehash=/home/hadoop/xa/runJar/conf/nodehash.properties -classpath ${runJar}/${jar} $main > /data/log/yangbotest/allLOg 2>&1 &
