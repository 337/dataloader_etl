 ...
 */
public class CoinInitialStateCacheMap {
    private static Log LOG= LogFactory.getLog(CoinInitialStateCacheMap.class);
    private static CoinInitialStateCacheMap instance;
    private Map<String,LongIntOpenHashMap> map;
    //private Map<String,Map<String,Connection>> cons;
    public synchronized static CoinInitialStateCacheMap getInstance(){
        if(instance==null){
            instance=new CoinInitialStateCacheMap();
        }
        return instance;
    }
    public CoinInitialStateCacheMap(){
    ...
    public int getVal(String project,long ssuid){
        ...
        if(!pidMap.containsKey(ssuid)){
            ...
            try {
                ...
                String queryStat="select val from coin_initialstate where uid="+samplingSeqUid;
        ...
        for(Event event: eventList){
            ...
            try {
                ...
                if(!projectMap.containsKey(samplingSeqUid)){
                    ...
                    if(sql==null){
                        sql="select * from coin_initialstatus where uid in ("+samplingSeqUid;
